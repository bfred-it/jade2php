...
(function() {
  ...
  var Compiler, ELSE_IF_REGEX, IF_REGEX, LOOP_REGEX, ONLY_PHP_VARIABLE_NAME, PHP_VARIABLE_NAME_BOOL, constantinople, doctypes, errorAtNode, filters, isConstant, jsExpressionToPhp, nodes, parseJSExpression, phpExtractorCode, phpRuntimeCode, runtime, selfClosing, toConstant, utils;
  ...
  ONLY_PHP_VARIABLE_NAME = /^\$[a-zA-Z_\x7f-\xff][a-zA-Z0-9_\x7f-\xff]*$/;
  PHP_VARIABLE_NAME_BOOL = /^([!]?)(\$[a-zA-Z_\x7f-\xff][a-zA-Z0-9_\x7f-\xff]*)$/;
  ...
  Compiler.prototype = {
    ...
    /**
    Verify variable existence in conditions
     */
    parseCondition: function (condition) {
      condition = this.jsExpressionToPhp(condition);
      var matches = condition.match(PHP_VARIABLE_NAME_BOOL);
      if (matches) {
        if (matches[1]) {
          condition = matches[1]+'isset('+matches[2]+') || '+matches[1]+matches[2];
        } else {
     ...
     */
    compile: function() {
      var e, i, mixin, mixinNames, result, x, y;
      try {
        this.buf = [];
        if (this.pp) {
      ...
      if (interpolate) {
        ...
        if (match) {
          ...
          if (match[1]) {
            ...
            if ("!" === match[2]) {
              code = "<?= " + (this.jsExpressionToPhp(range.src)) + " ?>";
            } else {
              code = "<?= htmlspecialchars(" + (this.jsExpressionToPhp(range.src)) + ") ?>";
            }
    ...
    visitDoctype: function(doctype) {
      ...
      if (this.doctype) {
        if (/<\?/.test(this.doctype)) {
          this.buf.push("<?= \'" + (this.doctype.replace("'", "\'")) + "\' ?>");
        } else {
    ...
    visitLoop: function(loopNode) {
      ...
      loopType = m[1];
      conditions = m[2];
      this.buf.push("<?php " + loopType + " (" + this.parseCondition(conditions) + ") : ?>");
      if (loopNode.block) {
    ...
    visitIf: function(ifNode) {
      ...
      this.buf.push("<?php if (" + this.parseCondition(ifNode.condition) + ") : ?>");
      if (ifNode.block) {
      ...
      if (!ifNode.nextElses) {
        ...
        for (k = 0, len1 = ref.length; k < len1; k++) {
          ...
          if (alternative.val === "else") {
            ...
            condition = m[1];
            this.buf.push("<?php elseif (" + this.parseCondition(condition) + ") : ?>");
            this.visit(alternative.block);
          }
        }
        return this.buf.push("<?php endif ?>");

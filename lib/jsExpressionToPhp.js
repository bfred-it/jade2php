...
(function() {
  ...
  jsExpressionToPhp = function(jsExpr, opts) {
    ...
    function addAccessor (phpExpression) {
      return phpExpression.replace(/(\$[a-zA-Z_][a-z_A-Z0-9]*)->([a-zA-Z_][a-z_A-Z0-9]*)/g, "access(\$1, '\$2')");
    }
    if (opts.arraysOnly && /^(!)?\s*\$?[a-zA-Z_][a-z_A-Z0-9]*/.test(phpExpression)) {
      var unset = phpExpression.match(/unset\((\$[a-zA-Z_][a-z_A-Z0-9]*)->([a-zA-Z_][a-z_A-Z0-9]*)\)/);
      var equals = phpExpression.match(/(\$[a-zA-Z_][a-z_A-Z0-9]*)->([a-zA-Z_][a-z_A-Z0-9]*) = (.*)/);
      if (unset) {
        phpExpression = 'if(is_array('+unset[1]+'))unset('+unset[1]+'["'+unset[2]+'"]);else unset('+unset[1]+'->'+unset[2]+')';
      } else if (equals) {
        equals[3] = addAccessor(equals[3]);
        phpExpression = 'if(is_array('+equals[1]+'))'+equals[1]+'["'+equals[2]+'"] = '+equals[3]+';else '+equals[1]+'->'+equals[2]+' = '+equals[3];
      } else {
        phpExpression = addAccessor(phpExpression);
      }
    }
